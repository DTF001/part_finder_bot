...
def handle_text(m):
    ...
    def checker(company, master, slave, your_pn, inv_qry, columns, warehouses, mat_type, precision):
        ...
        result = find_machine.trax_inventory_explorer(inv_qry=inv_qry, columns=columns, warehouses=warehouses, found=found)
        if not result.empty:
            ...
            extra_found = find_machine.extra_finder(slave=slave, mat_type=mat_type, precision=precision, your_pn=your_pn)
            if extra_found:
                ...
                result = find_machine.trax_inventory_explorer(inv_qry=inv_qry, columns=columns, found=extra_found_extracted,
    ...
    vdt = checker(company='ВДТМ', master=dataset_vdtm.pns_vdtm, slave=dataset_vdtm.pns_vdtm_int, your_pn=requested_pn, inv_qry=dataset_vdtm.inv_qry_vdtm,
                  columns=dataset_vdtm.columns_vdtm, warehouses=dataset_vdtm.wh_list_vdtm, mat_type=dataset_vdtm.pn_vdtm_category, precision=75)
    airlines = checker(company='ABC и ATRAN', master=dataset_vd_airlines.pns_airlines, slave=dataset_vd_airlines.pns_airlines_int, your_pn=requested_pn,
                       inv_qry=dataset_vd_airlines.inv_qry_airlines, columns=dataset_vd_airlines.columns_airlines, warehouses=dataset_vd_airlines.wh_list_airlines,
                       mat_type=dataset_vd_airlines.pn_airlines_category, precision=75)
